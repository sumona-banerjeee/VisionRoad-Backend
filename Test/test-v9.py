...
# ===================== ROI =====================
# Expanded ROI to detect both signboards (upper) and potholes (lower)
ROI_LEFT = int(width * 0.0)
ROI_RIGHT = int(width * 1.0)
ROI_TOP = int(height * 0.05)  # Start from 5% to capture signboards
ROI_BOTTOM = int(height * 0.95)  # Extend to 90% to capture potholes on road
...
while processing:
    ...
    results = model.track(
        current_frame, persist=True, conf=CONF_THRESHOLD, tracker=TRACKER, verbose=False
    )
...
output_json["summary"]["unique_potholes"] = int(len(counted_potholes))
output_json["summary"]["unique_signboards"] = int(len(counted_signboards))
...
print(f"Unique Potholes Detected: {len(counted_potholes)}")
print(f"Unique Signboards Detected: {len(counted_signboards)}")
...
if counted_potholes:
    print(f"\nConfirmed Potholes ({len(counted_potholes)}):")
    for pot in output_json["pothole_list"]:
        print(
            f"  ID {pot['detection_id']}: {pot['type']} "
            f"(Frame {pot['first_detected_frame']}, "
            f"Time {pot['first_detected_time']}s, "
            f"Conf {pot['confidence']})"
        )

if counted_signboards:
    print(f"\nConfirmed Signboards ({len(counted_signboards)}):")
    for sb in output_json["signboard_list"]:
        print(
            f"  ID {sb['detection_id']}: {sb['type']} "
