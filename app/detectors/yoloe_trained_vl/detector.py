"""
YoloeTrainedVlDetector — Isolated fine-tuned YOLOE detector.
User explicitly requested NOT to touch app/detectors/yolo/detector.py.
Therefore, the processing method is duplicated and modified here.
"""
"""
...
class YoloeTrainedVlDetector(YoloDetector):
    def __init__(self):
        ...
        self.conf_threshold = YOLOE_TRAINED_CONF
    ...
    def is_duplicate_location(self, *args, **kwargs):
        # spatial deduplication removed for yoloe with vl
        return False, None
    ...
    def _process_video_blocking(
        ...
        try:
            ...
            while cap.isOpened():
                ...
                if results[0].boxes.id is not None:
                    ...
                    for tid, cid, box, conf in zip(
                        ...
                        pass  # ROI check disabled for YOLOE
            ...
            for _fdata in _pending_frames:
                ...
                if _surviving:
                    # FRAME LABEL BUG REPAIR: Backfill true VL names
                    for d in _surviving:
