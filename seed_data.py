...
def create_chainage(
    ...
    if response.status_code == 201:
        data = response.json()
        print(f"    ✓ Created chainage: {segment_name} [KM {chainage_start_km}–{chainage_end_km}] (ID: {data['id']})")
        return data
    else:
...
def create_lane(chainage_id, lane_code, lane_type=None, direction=None):
    """Create a lane via API"""
    response = requests.post(
        f"{BASE_URL}/lanes",
        json={
    ...
    if response.status_code == 201:
        data = response.json()
        print(f"      ✓ Created lane: {lane_code} (ID: {data['id']})")
        return data
    else:
        print(f"      ✗ Failed to create lane {lane_code}: {response.text}")
        return None


def add_default_lanes(chainage_id):
    """Add default LHS and RHS lanes to a chainage"""
    create_lane(chainage_id, "LHS", lane_type="driving", direction="Left Hand Side")
    create_lane(chainage_id, "RHS", lane_type="driving", direction="Right Hand Side")
...
def seed_kolkata_jaipur_highway():
    ...
    if package1:
        ch = create_chainage(package1["id"], "Kolkata South Corridor", 0, 10,
                             22.569, 88.430, 22.582, 88.438)
        if ch: add_default_lanes(ch["id"])

        ch = create_chainage(package1["id"], "Dankuni-Burdwan", 10, 80,
                             22.6500, 88.2800, 23.2500, 87.8600)
        if ch: add_default_lanes(ch["id"])

        ch = create_chainage(package1["id"], "Burdwan-Asansol", 80, 160,
                             23.2500, 87.8600, 23.6850, 86.9830)
        if ch: add_default_lanes(ch["id"])

        ch = create_chainage(package1["id"], "Asansol-Dhanbad", 160, 220,
                             23.6850, 86.9830, 23.7957, 86.4304)
        if ch: add_default_lanes(ch["id"])
    ...
    if package2:
        ch = create_chainage(package2["id"], "Patna-Aurangabad", 400, 500,
                             25.5941, 85.1376, 24.7521, 84.3742)
        if ch: add_default_lanes(ch["id"])

        ch = create_chainage(package2["id"], "Aurangabad-Varanasi", 500, 600,
                             24.7521, 84.3742, 25.3176, 82.9739)
        if ch: add_default_lanes(ch["id"])
    ...
    if package3:
        ch = create_chainage(package3["id"], "Varanasi-Allahabad", 600, 720,
                             25.3176, 82.9739, 25.4358, 81.8463)
        if ch: add_default_lanes(ch["id"])

        ch = create_chainage(package3["id"], "Allahabad-Kanpur", 720, 900,
                             25.4358, 81.8463, 26.4499, 80.3319)
        if ch: add_default_lanes(ch["id"])
...
def seed_mumbai_pune_expressway():
    ...
    if package1:
        ch = create_chainage(package1["id"], "Kalyan-Khopoli", 0, 40,
                             19.2403, 73.1305, 18.7854, 73.3457)
        if ch: add_default_lanes(ch["id"])

        ch = create_chainage(package1["id"], "Khopoli-Lonavala", 40, 70,
                             18.7854, 73.3457, 18.7537, 73.4087)
        if ch: add_default_lanes(ch["id"])
    ...
    if package2:
        ch = create_chainage(package2["id"], "Lonavala-Talegaon", 70, 85,
                             18.7537, 73.4087, 18.7351, 73.6765)
        if ch: add_default_lanes(ch["id"])

        ch = create_chainage(package2["id"], "Talegaon-Pune", 85, 95,
                             18.7351, 73.6765, 18.5204, 73.8567)
        if ch: add_default_lanes(ch["id"])
...
def seed_delhi_chandigarh_highway():
    ...
    if package1:
        ch = create_chainage(package1["id"], "Delhi-Sonipat", 0, 45,
                             28.7041, 77.1025, 28.9931, 77.0151)
        if ch: add_default_lanes(ch["id"])

        ch = create_chainage(package1["id"], "Sonipat-Panipat", 45, 90,
                             28.9931, 77.0151, 29.3909, 76.9635)
        if ch: add_default_lanes(ch["id"])
    ...
    if package2:
        ch = create_chainage(package2["id"], "Panipat-Karnal", 90, 150,
                             29.3909, 76.9635, 29.6857, 76.9905)
        if ch: add_default_lanes(ch["id"])

        ch = create_chainage(package2["id"], "Karnal-Ambala", 150, 220,
                             29.6857, 76.9905, 30.3782, 76.7821)
        if ch: add_default_lanes(ch["id"])

        ch = create_chainage(package2["id"], "Ambala-Chandigarh", 220, 270,
                             30.3782, 76.7821, 30.7333, 76.7794)
        if ch: add_default_lanes(ch["id"])
...
def main():
    ...
    print(f"  - List lanes:      GET {BASE_URL}/lanes")
